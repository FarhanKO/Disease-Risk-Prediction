...
metrics:
  ...
  - roc_auc
model-index:
  - name: heart-ecg-resnet50v2
    results:
      - task:
          type: image-classification
          name: 12-lead ECG printout classification (4 classes)
        dataset:
          name: ECG Images dataset of Cardiac Patients (deduplicated, test split)
          type: ecg-images-cardiac-patients
        metrics:
          - type: accuracy
            value: 0.878
            name: Test accuracy
...
- **Tiny MI class.** There are only 30 distinct Myocardial Infarction ECGs,
  and 4 of them are in the test set. Per-class MI numbers are only indicative.
- **One source.** All ECGs come from one hospital, one ECG machine and one
  print template. Other machines, lead layouts, paper speeds, phone photos or
  digital ECG exports are untested, and the OOD gate may reject them.
- **Split by ECG, not by patient.** The dataset does not publish patient IDs,
  so different ECGs from the same patient may appear in both train and test.
  Only exact duplicates were removed.
- **Coarse labels.** The dataset does not give the arrhythmia type, the
  location of the infarct, or how long ago a past MI happened.
- **Possible shortcut.** For some Normal and Abnormal Heartbeat ECGs, the
  Grad-CAM heat sits on the right edge of the printout rather than on the
  waveform. For MI, it sits on the ST-elevated chest leads, as expected.

## Citation

If you use these models, please cite the dataset:
...
The model weights are released under CC BY 4.0, the same licence as the
training data.
